...
pub async fn preflight_prices(
    all_autopool_constants: &Vec<AutopoolAddressConstants>,
    multicall3: Address,
...
pub async fn preflight_autopool_constants(
    autopools: Vec<Address>,
    multicall3: Address,
    provider: RootProvider,
    block: u64,
) -> Result<(EthEvmInput, Vec<AutopoolAddressConstants>), anyhow::Error> {
    let mut env = EthEvmEnv::builder()
        .provider(provider)
        .block_number(block)
        .chain_spec(&ETH_MAINNET_CHAIN_SPEC)
    ...
    let all_autopool_constants = {
        let mut all_autopool_constants = Vec::with_capacity(autopools.len());
        ...
        for autopool in autopools {
            let autopool_constants = {
                let (destination_vaults, base_asset, system_registry, root_price_oracle) = {
                    let (destination_vaults, base_asset, system_registry) = {
                        let calls: Vec<IMulticall3::Call3> = vec![
                            IMulticall3::Call3 {
                                target: autopool,
                                allowFailure: false,
                                callData: IMinimalAutoPool::getDestinationsCall {}
                                    .abi_encode()
                                    .into(),
                            },
                            IMulticall3::Call3 {
                    ...
                    let mut system_registry_contract =
                        Contract::preflight(system_registry, &mut env);
                ...
                let destination_vault_keys = {
                    ...
                    let destination_vault_keys = {
                        let mut destination_vault_keys =
                        ...
                        for (index, destination_vault) in destination_vaults.iter().enumerate() {
                            ...
                            let pool: Address =
                                IMinimalDestinationVault::getPoolCall::abi_decode_returns(
                                    &pool_result.returnData,
                                )?
                                .into();

                            let token: Address =
                                IMinimalDestinationVault::underlyingCall::abi_decode_returns(
    ...
    let input = env.into_input().await?;
    Ok((input, all_autopool_constants))
}
